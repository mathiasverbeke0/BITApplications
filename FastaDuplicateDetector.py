...
# Usage: This script checks whether a multifasta file has duplicate FASTA definition lines
...
# make a list which will contain all the UNIQUE accession FASTA definition lines
accession_list = []

# make a list which will contain all of the UNIQUE FASTA definition lines and corresponding sequences
unique_fasta = []

# create 2 flags
# flag will include/exclude unique/duplicate FASTA definition lines and corresponding sequences
...
for line in file.readlines():
    ...
    if line[0] == ">":
        ...
        # if the FASTA definition line is unique (not yet in the list) it will be added to the list
        if line not in accession_list:
            unique_fasta.append(line)
            accession_list.append(line)
            flag = "OK"
        
        # if the FASTA definition line is not unique (already in the list) it will be exluded from the list. 
        # the flag becomes NOK, meaning the sequences belonging to the FASTA definition line will not be processed (stored in the unique_fasta list)
        else:
            flag = "NOK"
            duplicate_flag = "Yes"
            continue
    ...
    # the sequences belonging to duplicate FASTA definition lines will not be processed (stored in the unique_fasta list)    
    elif flag == "NOK":
        continue
